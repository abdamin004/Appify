    ...
    >
      ...
      {!editOnly && bazaars.length > 0 && (
        <div className="mt-16 pt-10 border-t border-slate-200">
          <h2 className="text-2xl font-bold text-slate-800 mb-6">Existing Bazaars</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {bazaars.map((bz) => (
              <div
                key={bz._id}
              ...
              >
                <div className="flex justify-between items-start mb-4">
                  ...
                  <span className={`px-2 py-1 rounded text-xs font-bold uppercase ${bz.status === 'published' ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600'
                    }`}>
                    {bz.status}
                  </span>
                </div>
