...
function WorkshopParticipantsView({ workshops }) {
  ...
  const getStatusColor = (status) => {
    switch (status?.toLowerCase()) {
      case 'pending':
        return { bg: 'bg-amber-50', border: 'border-amber-200', text: 'text-amber-600', label: 'Pending Approval' };
      case 'published':
        return { bg: 'bg-emerald-50', border: 'border-emerald-200', text: 'text-emerald-600', label: 'Published' };
      case 'rejected':
        return { bg: 'bg-red-50', border: 'border-red-200', text: 'text-red-600', label: 'Rejected' };
      case 'draft':
        return { bg: 'bg-slate-100', border: 'border-slate-300', text: 'text-slate-500', label: 'Draft' };
      default:
        return { bg: 'bg-slate-100', border: 'border-slate-300', text: 'text-slate-500', label: status || 'Unknown' };
    }
  };
  ...
  if (!workshops || !Array.isArray(workshops) || workshops.length === 0) {
    return (
      <div className="bg-white p-16 rounded-2xl text-center shadow-sm border border-slate-200">
    ...
    <div className="flex flex-col gap-6">
      {workshops.map((workshop) => {
        ...
        return (
          <div
            key={workshopId}
            className="bg-white rounded-2xl overflow-hidden shadow-sm transition-all hover:shadow-md border border-slate-200"
          >
            <div className={`p-6 ${isOpen ? 'border-b border-slate-100' : ''}`}>
              <div className="flex justify-between items-center flex-wrap gap-4">
                <div className="flex-1 min-w-[200px]">
                  <div className="flex items-center gap-3 mb-2 flex-wrap">
                    ...
                    {hasEditRequests && (
                      <div className="px-3 py-1 bg-amber-50 border border-amber-200 rounded-xl text-xs font-bold text-amber-600 flex items-center gap-1.5">
                        <span>✏️</span>
                  ...
                  <div className="flex flex-col gap-1.5 text-sm text-slate-500">
                    ...
                    {remainingSpots !== null && (
                      <div className="flex items-center gap-2">
                        <span>✅</span>
                        <span>
                          Remaining Spots: <strong className={remainingSpots > 0 ? "text-emerald-500" : "text-red-500"}>
                            {remainingSpots}
                          </strong>
                        </span>
                      </div>
                ...
                <div className="flex gap-3 flex-wrap">
                  <button
                    ...
                    className="px-6 py-3 bg-slate-900 text-white border-none rounded-xl text-sm font-bold cursor-pointer transition-all shadow-sm hover:bg-emerald-600 hover:-translate-y-0.5 hover:shadow-md"
                  >
                    ✏️ Edit Workshop
                  </button>
                  <button
            ...
            {hasEditRequests && (
              <div className="p-6 bg-amber-50/50 border-t border-amber-200">
                <div className={`flex justify-between items-center ${showEditRequestsForThis ? 'mb-4' : ''}`}>
                  <div className="flex items-center gap-2.5">
                    <span className="text-xl">✏️</span>
                    <h4 className="text-base font-bold text-amber-700 m-0">
                      Edit Requests from Events Office ({editRequests.length})
                    </h4>
                  </div>
                  <button
                    onClick={() => setShowEditRequests(prev => ({ ...prev, [workshopId]: !prev[workshopId] }))}
                    className={`px-4 py-2 border rounded-lg text-sm font-semibold text-amber-700 cursor-pointer transition-all ${showEditRequestsForThis
                      ? "bg-amber-100 border-amber-300"
                      : "bg-transparent border-amber-300 hover:bg-amber-50"
                      }`}
                  >
                ...
                {showEditRequestsForThis && (
                  <div className="flex flex-col gap-3">
                    ...
                    <div className="mt-2">
                      <button
                        onClick={() => navigate(`/professor/workshops?edit=${workshopId}`)}
                        className="px-5 py-2.5 bg-amber-500 text-white border-none rounded-lg text-sm font-bold cursor-pointer transition-all hover:bg-amber-600 hover:-translate-y-0.5"
                      >
            ...
            {isOpen && (
              <div className="p-6 bg-slate-50 border-t border-slate-200">
                {data?.loading ? (
                  ...
                  <div className="text-center p-6 text-red-500 bg-red-50 rounded-xl border border-red-200">
                  ...
                  <div>
                    ...
                    <div className="flex flex-col gap-3 max-h-[400px] overflow-y-auto pr-2">
                      {data.participants.map((participant, index) => (
                        ...
                        >
                          ...
                          <div className="px-3 py-1.5 bg-slate-100 text-slate-600 rounded-lg text-sm font-semibold">
                            #{index + 1}
                          </div>
                        </div>
                      ))}
                    ...
                    {data.remainingSpots !== null && (
                      <div className={`mt-5 p-4 rounded-xl border text-center ${data.remainingSpots > 0
                        ...
                        <div className={`text-lg font-bold ${data.remainingSpots > 0 ? "text-emerald-600" : "text-red-600"
                          }`}>
